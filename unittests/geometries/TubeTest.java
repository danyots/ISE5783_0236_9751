...
class TubeTest {
    ...
    public void testGetNormal() {
        ...
        // TC01: There is a simple single test here - a GeoPoint and an axis
    ...
    /**
     * Test method for {@link geometries.Tube#findGeoIntersectionsHelper(Ray)} (primitives.GeoPoint)}.
     */
    @Test
    void findGeoIntersectionsHelper() {
        ...
        Ray ray;
        // ============ Equivalence Partitions Tests ==============
        // TC01: Ray's line is outside the tube (0 GeoPoints)
        ray = new Ray(new Point(1, 1, 2), new Vector(1, 1, 0));
        assertNull(tube1.findGeoIntersectionsHelper(ray), "ERROR: Ray's line out of tube");

        // TC02: Ray's crosses the tube (2 GeoPoints)
        ray = new Ray(new Point(0, 0, 0), new Vector(2, 1, 1));
        List<GeoPoint> result = tube2.findGeoIntersectionsHelper(ray);
        ...
        assertEquals(
                List.of(new GeoPoint(tube2,new Point(0.4, 0.2, 0.2)),
                        new GeoPoint(tube2,new Point(2, 1, 1))), result, "ERROR - TC02: Bad intersections");

        // TC03: Ray's starts within tube and crosses the tube (1 GeoPoint)
        ...
        assertEquals(List.of(new GeoPoint(tube2,new Point(2, 1, 1))), result, "ERROR - TC03: Bad intersections");

        // =============== Boundary Values Tests ==================

        // **** Group: Ray's line is parallel to the axis (0 GeoPoints)

        // TC11: Ray is inside the tube (0 GeoPoints)
        ...
        // TC13: Ray is at the tube surface (0 GeoPoints)
        ray = new Ray(new Point(2, 1, 0.5), v2);
        assertNull(tube2.findGeoIntersectionsHelper(ray), "must not be intersections");

        // TC14: Ray is inside the tube and starts against axis head (0 GeoPoints)
        ray = new Ray(new Point(0.5, 0.5, 1), v2);
        assertNull(tube2.findGeoIntersectionsHelper(ray), "must not be intersections");

        // TC15: Ray is outside the tube and starts against axis head (0 GeoPoints)
        ray = new Ray(new Point(0.5, -0.5, 1), v2);
        assertNull(tube2.findGeoIntersectionsHelper(ray), "must not be intersections");

        // TC16: Ray is at the tube surface and starts against axis head (0 GeoPoints)
        ray = new Ray(new Point(2, 1, 1), v2);
        assertNull(tube2.findGeoIntersectionsHelper(ray), "must not be intersections");

        // TC17: Ray is inside the tube and starts at axis head (0 GeoPoints)
        ray = new Ray(new Point(1, 1, 1), v2);
        assertNull(tube2.findGeoIntersectionsHelper(ray), "must not be intersections");

        // **** Group: Ray is orthogonal but does not begin against the axis head

        // TC21: Ray starts outside and the line is outside (0 GeoPoints)
        ray = new Ray(new Point(0, 2, 2), new Vector(1, 1, 0));
        assertNull(tube2.findGeoIntersectionsHelper(ray), "must not be intersections");

        // TC22: The line is tangent and the ray starts before the tube (0 GeoPoints)
        ray = new Ray(new Point(0, 2, 2), new Vector(1, 0, 0));
        assertNull(tube2.findGeoIntersectionsHelper(ray), "must not be intersections");

        // TC23: The line is tangent and the ray starts at the tube (0 GeoPoints)
        ray = new Ray(new Point(1, 2, 2), new Vector(1, 0, 0));
        assertNull(tube2.findGeoIntersectionsHelper(ray), "must not be intersections");

        // TC24: The line is tangent and the ray starts after the tube (0 GeoPoints)
        ray = new Ray(new Point(2, 2, 2), new Vector(1, 0, 0));
        assertNull(tube2.findGeoIntersectionsHelper(ray), "must not be intersections");

        // TC25: Ray starts before (2 GeoPoints)
        ...
        if (result.get(0).point.getY() > result.get(1).point.getY())
            result = List.of(result.get(1), result.get(0));
        assertEquals(List.of(new GeoPoint(tube2,new Point(0.4, 0.2, 2)), new GeoPoint(tube2,new Point(2, 1, 2))), result, "Bad intersections");

        // TC26: Ray starts at the surface and goes inside (1 GeoPoint)
        ray = new Ray(new Point(0.4, 0.2, 2), new Vector(2, 1, 0));
        result = tube2.findGeoIntersectionsHelper(ray);
        assertNotNull(result, "must be intersections");
        assertEquals(1, result.size(), "must be 1 intersections");
        assertEquals(List.of(new GeoPoint(tube2,new Point(2, 1, 2))), result, "Bad intersections");

        // TC27: Ray starts inside (1 GeoPoint)
        ...
        // TC28: Ray starts at the surface and goes outside (0 GeoPoints)
        ray = new Ray(new Point(2, 1, 2), new Vector(2, 1, 0));
        result = tube2.findGeoIntersectionsHelper(ray);
        assertNull(result, "Bad intersections");

        // TC29: Ray starts after (0 GeoPoints)
        ray = new Ray(new Point(4, 2, 2), new Vector(2, 1, 0));
        result = tube2.findGeoIntersectionsHelper(ray);
        assertNull(result, "Bad intersections");

        // TC30: Ray starts before and crosses the axis (2 GeoPoints)
        ...
        if (result.get(0).point.getY() > result.get(1).point.getY())
            result = List.of(result.get(1), result.get(0));
        assertEquals(List.of(new GeoPoint(tube2,new Point(1, 0, 2)), new GeoPoint(tube2,new Point(1, 2, 2))), result, "Bad intersections");

        // TC31: Ray starts at the surface and goes inside and crosses the axis (1 GeoPoint)
        ray = new Ray(new Point(1, 0, 2), new Vector(0, 1, 0));
        result = tube2.findGeoIntersectionsHelper(ray);
        assertNotNull(result, "must be intersections");
        assertEquals(1, result.size(), "must be 1 intersections");
        assertEquals(List.of(new GeoPoint(tube2,new Point(1, 2, 2))), result, "Bad intersections");

        // TC32: Ray starts inside and the line crosses the axis (1 GeoPoint)
        ...
        // TC33: Ray starts at the surface and goes outside and the line crosses the axis (0 GeoPoints)
        ray = new Ray(new Point(1, 2, 2), new Vector(0, 1, 0));
        result = tube2.findGeoIntersectionsHelper(ray);
        assertNull(result, "Bad intersections");

        // TC34: Ray starts after and crosses the axis (0 GeoPoints)
        ray = new Ray(new Point(1, 3, 2), new Vector(0, 1, 0));
        result = tube2.findGeoIntersectionsHelper(ray);
        assertNull(result, "Bad intersections");

        // TC35: Ray start at the axis (0 GeoPoints)
        ray = new Ray(new Point(1, 1, 2), new Vector(0, 1, 0));
        result = tube2.findGeoIntersectionsHelper(ray);
        assertNotNull(result, "must be intersections");
        assertEquals(1, result.size(), "must be 1 intersections");
        ...
        if (result.get(0).point.getY() > result.get(1).point.getY())
            result = List.of(result.get(1), result.get(0));
        assertEquals(List.of(new GeoPoint(tube2,new Point(0.4, 0.2, 1)), new GeoPoint(tube2,new Point(2, 1, 1))), result, "Bad intersections");
        ...
        assertNotNull(result, "must be intersections");
        assertEquals(1, result.size(), "must be 1 intersections");
        assertEquals(List.of(new GeoPoint(tube2,new Point(2, 1, 1))), result, "Bad intersections");
        ...
        if (result.get(0).point.getY() > result.get(1).point.getY())
            result = List.of(result.get(1), result.get(0));
        assertEquals(List.of(new GeoPoint(tube2,new Point(1, 0, 1)), new GeoPoint(tube2,new Point(1, 2, 1))), result, "Bad intersections");
        ...
        assertNotNull(result, "must be intersections");
        assertEquals(1, result.size(), "must be 1 intersections");
        assertEquals(List.of(new GeoPoint(tube2,new Point(1, 2, 1))), result, "Bad intersections");
        ...
        if (result.get(0).point.getY() > result.get(1).point.getY())
            result = List.of(result.get(1), result.get(0));
        assertEquals(List.of(new GeoPoint(tube2,new Point(2, 1, 2)), new GeoPoint(tube2,new Point(0.4, 1.8, 1.2))), result, "Bad intersections");
        ...
        assertNotNull(result, "must be intersections");
        assertEquals(1, result.size(), "must be 1 intersections");
        assertEquals(List.of(new GeoPoint(tube2,new Point(2, 1, 1.8))), result, "Bad intersections");
        ...
        assertNotNull(result, "must be intersections");
        assertEquals(1, result.size(), "must be 1 intersections");
        assertEquals(List.of(new GeoPoint(tube2,new Point(2, 1, 1.5))), result, "Bad intersections");
        ...
        if (result.get(0).point.getY() > result.get(1).point.getY())
            result = List.of(result.get(1), result.get(0));
        assertEquals(List.of(new GeoPoint(tube2,new Point(value1, value1, 2 + value1)), new GeoPoint(tube2,new Point(value2, value2, 2 + value2))),
        ...
        if (result.get(0).point.getY() > result.get(1).point.getY())
            result = List.of(result.get(1), result.get(0));
        assertEquals(List.of(new GeoPoint(tube2,new Point(value1, value1, value1)), new GeoPoint(tube2,new Point(value2, value2, value2))),
                result, "Bad intersections");
        ...
        assertNotNull(result, "must be intersections");
        assertEquals(1, result.size(), "must be 1 intersections");
        assertEquals(List.of(new GeoPoint(tube2,new Point(value2, value2, 2 + value2))), result, "Bad intersections");
        ...
        assertNotNull(result, "must be intersections");
        assertEquals(1, result.size(), "must be 1 intersections");
        assertEquals(List.of(new GeoPoint(tube2,new Point(value2, value2, value2))), result, "Bad intersections");
